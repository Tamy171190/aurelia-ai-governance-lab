...
Every material vendor finding should identify the relevant requirement theme for:
...
The assessment should not assume that a framework applies solely because a country appears in the vendor's footprint. Applicability is assessed using the AI system, activity, entity, data flow, provider/deployer role and geography.

## Control Ceiling

Where several applicable frameworks address the same supplier risk, Aurelia uses the **highest applicable control requirement** as the baseline where practical. Jurisdiction-specific clauses are retained where necessary.

Example: if model update notification, incident notification, audit rights or data-processing requirements differ by jurisdiction, the vendor contract and monitoring standard should adopt the strongest applicable requirement rather than creating avoidable parallel processes.
